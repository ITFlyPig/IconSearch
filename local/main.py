...
from local.src.file_sync import sync_all_img
...
if __name__ == '__main__':
    ...
    # 将图片中的vector drawable、 ssvg转为一般的图片格式，并保存到新的文件目录中
    handle_images(config.image.origin_save_dir, config.image.handled_dir)
    # 将处理好的图片同步到服务器
    logger.info('start sync image to server')
    sync_all_img(config.image.handled_dir, config.image.remote_img_dir)
